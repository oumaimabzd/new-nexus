...
const courseConfig = {
  ...
  title: "School Safety — Risks and Challenges",
  role: "teacher",
...
function init() {
  ...
  // Stepper can be injected by sidebar.js. On refresh it may not exist yet.
  // Render now (header/content), then render + wire stepper as soon as it mounts.
  onStepperReady(() => {
    wireStepperKeyboard();
    renderStepper(appState);
  });
...
/* === SN SIDEBAR INJECTION (merged from sidebar.js) ===
   Purpose: allow deleting sidebar.js while keeping identical behavior.
   Notes:
   - Injects the stepper panel markup (course progress rail) into <div data-sn-sidebar></div>
     or, if missing, inserts after .navRail inside .layout.
   - Keeps the SAME ids/hooks your course.js already uses (#stepper, #courseTitle, etc.)
   - Adds Reset progress (demo) wiring here (since sidebar.js is removed).
========================================================= */
(function snInjectSidebar() {
  "use strict";
      ...
      <div class="utilityPanel__inner">
        ...
        <div class="stepperCard" role="group" aria-labelledby="stepperLabel">
          ...
          <p class="caption stepperHelp" id="stepperHelp">
            Tip: You may jump between steps. If you skip ahead before saving your quiz answers, we’ll gently remind you.
          </p>
        </div>
  ...
  function wireResetProgressButton() {
    ...
    btn.addEventListener("click", () => {
      try {
        ...
        localStorage.removeItem(STORAGE.answers(courseConfig.courseId));
        localStorage.removeItem(STORAGE.watched(courseConfig.courseId));
        localStorage.removeItem(STORAGE.quizSeq(courseConfig.courseId));

        localStorage.removeItem(QUIZ_STORAGE.q1_answers(courseConfig.courseId));
        localStorage.removeItem(QUIZ_STORAGE.q1_savedAt(courseConfig.courseId));
        localStorage.removeItem(QUIZ_STORAGE.q1_locked(courseConfig.courseId));

        localStorage.removeItem(QUIZ_STORAGE.q2_answers(courseConfig.courseId));
        localStorage.removeItem(QUIZ_STORAGE.q2_savedAt(courseConfig.courseId));
        localStorage.removeItem(QUIZ_STORAGE.q2_locked(courseConfig.courseId));
        localStorage.removeItem(
          QUIZ_STORAGE.q2_analysis(courseConfig.courseId),
        );

        localStorage.removeItem(QUIZ_STORAGE.q3_matches(courseConfig.courseId));
        localStorage.removeItem(QUIZ_STORAGE.q3_savedAt(courseConfig.courseId));
        localStorage.removeItem(QUIZ_STORAGE.q3_locked(courseConfig.courseId));
      } catch (e) {}
